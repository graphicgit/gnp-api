...
public:
  ...
  [[nodiscard]] const std::string &getPublicationId() const {
    return publication_id_;
  }
  [[nodiscard]] const std::string &getPublicationName() const {
    return publication_name_;
  }
  [[nodiscard]] const std::string &getEditionNumber() const {
    return edition_number_;
  }
  [[nodiscard]] bool getIsPopular() const { return is_popular_; }
  [[nodiscard]] const std::string &getFullDescription() const {
    return full_description_;
  }
  [[nodiscard]] const std::string &getThumbnailId() const {
    return thumbnail_id_;
  }
  [[nodiscard]] const std::string &getFileType() const { return file_type_; }
  [[nodiscard]] const std::string &getStorageService() const {
    return storage_service_;
  }
  [[nodiscard]] const std::string &getDocumentId() const {
    return document_id_;
  }
  [[nodiscard]] bool isPublished() const { return is_published_; }
  [[nodiscard]] const ::trantor::Date &getPublicationDate() const {
    return publication_date_;
  }
  [[nodiscard]] const std::string &getFeaturedStories() const {
    return featured_stories_;
  }
