...
public:
  ...
  [[nodiscard]] const std::string &getTargetAudience() const {
    return target_audience_;
  }
  ...
  [[nodiscard]] const std::string &getMessageBody() const {
    return message_body_;
  }
  [[nodiscard]] const std::string &getStatus() const { return status_; }
  [[nodiscard]] const std::string &getCampaignType() const {
    return campaign_type_;
  }
  [[nodiscard]] int getEngagement() const { return engagement_; }
  [[nodiscard]] const ::trantor::Date &getScheduledTime() const {
...
inline void CreateCampaignDto::fromJson(const Json::Value &json) {
  ...
  if (json.isMember("scheduledTime") && !json["scheduledTime"].isNull()) {
    std::string scheduledTimeStr =
        json["scheduledTime"].asString(); // 2025-12-31T06:10
  ...
  if (json.isMember("htmlTemplate") && !json["htmlTemplate"].isNull()) {
    html_template_ = json["htmlTemplate"].asString();
  }
}
